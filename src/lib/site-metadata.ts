export const siteMetadata = {
	/**
	 * basic Information
	 */
	brand: `훈지`,
	title: `hooninedev.com`,
	description: `프론트엔드 개발자 이지훈(후니)의 기술 블로그. React, TypeScript, Next.js 로 만들면서 겪은 설계 판단과 문제 해결 과정, 성능과 관측 실측, 학습 노트를 남깁니다.`,
	language: `ko`,
	siteUrl: `https://hooninedev.com`,

	/**
	 * 글 이용 라이선스 (JSON-LD license 필드 등에서 사용)
	 * 크롤러/LLM에 글의 사용 조건을 명시한다.
	 */
	license: `https://creativecommons.org/licenses/by-nc/4.0/`,

	/**
	 * comments setting
	 */
	comments: {
		utterances: {
			repo: `jiji-hoon96/jihoon-blog`,
		},
	},

	/**
	 * introduce yourself
	 */
	author: {
		name: `이지훈`,
		nickname: `후니`,
		stack: ["React", "Typescript", "Javascript", "Zustand", "Tanstack"],
		bio: {
			email: `jihoon7705@gmail.com`,
			residence: "Seoul, South Korea",
		},
		social: {
			github: `https://github.com/jiji-hoon96`,
			linkedIn: `https://www.linkedin.com/in/jiji-hoon96`,
			resume: `https://www.figma.com/design/d1bKNaIiHzJYc94E1SvlrO/%EC%9D%B4%EC%A7%80%ED%9B%88-%EC%9D%B4%EB%A0%A5%EC%84%9C?node-id=0-1&t=B5TcORyy2IuzJBSU-1`,
		},
		dropdown: {
			tistory: "",
			velog: "",
		},
	},

	/**
	 * definition of featured posts
	 */
	featured: [
		{
			title: "프론트엔드",
			category: "프론트엔드",
		},
		{
			title: "소박한궁금증",
			category: "소박한궁금증",
		},
		{
			title: "대외활동",
			category: "대외활동",
		},
		{
			title: "일상",
			category: "일상",
		},
	],

	/**
	 * pinned posts on homepage (use post slug)
	 * Example: ['/240101', '/230816', '/240909']
	 */
	pinnedPosts: ['/260418', '/260201', '/250520'],

	/**
	 * metadata for About Page
	 */
	timestamps: [
		{
			category: "Career",
			date: "2023.03 - NOW",
			en: "HicareNet",
			kr: "하이케어넷",
			info: "Frontend Developer",
			link: "",
		},
		{
			category: "Career",
			date: "2022.10 - 2023.01",
			en: "Langcode",
			kr: "랭코드",
			info: "Fullstack Developer",
			link: "",
		},
		{
			category: "Activity",
			date: "2025.06 - NOW",
			en: "Nexters",
			kr: "넥스터즈",
			info: "Boolti, Satto",
			link: "",
		},
		{
			category: "Activity",
			date: "2023.06 - 2025.1",
			en: "SIPE",
			kr: "사이프",
			info: "IT 커뮤니티-운영진 & 회원",
			link: "",
		},
		{
			category: "Activity",
			date: "2024.06 - 2024.08",
			en: "DND",
			kr: "DND",
			info: "사이드 프로젝트 동아리-6조",
			link: "",
		},
	],

	/**
	 * metadata for Playground Page
	 */
	projects: [
		{
			title: "Portfolio",
			description: "포트폴리오",
			techStack: ["React", "Next.js", "Typescript"],
			thumbnailUrl: "",
			links: {
				post: "",
				github: "",
				demo: "",
				googlePlay: "",
				appStore: "",
			},
		},
	],
};
