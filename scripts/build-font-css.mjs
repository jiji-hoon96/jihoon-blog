 ...
 *
 * 더 큰 몫은 폰트 파일이다. 굵기마다 한글 서브셋을 따로 받으므로, 쓰지 않는 굵기를
 * CSS 에 남겨 두면 그 굵기의 서브셋까지 내려받는다. 글 페이지 실측으로 굵기 4종
 * 49개 파일에서 2종 31개로 줄었고 그 차이가 241,668 B 다.
 *
 ...
 *
 * CSS 바이트 자체의 절감은 작다. 양쪽 다 brotli 로 협상되므로 16,309 B 대
 * 11,116 B 다(실측). 아래 로그가 찍는 gzip 수치를 이 변경의 근거로 쓰지 않는다.
 *
 * 버전을 올릴 때 다시 돌린다.
