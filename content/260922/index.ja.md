...
筆者のデータでは失敗したので、ほかの人はどこに使っているのかを見た。[jevable.com](https://jevable.com/)は、Nikunj という開発者が X に投稿された Jev プロジェクトを審査して集めた独立のキュレーションだ。2026年9月23日時点で194件あり、登録日は9月16日から20日の間に集中している。152件が9月18日の一日に入ってきた。公開から三日後だ。
...
それでは何を作ってみる価値があるか。上の三つの形と前の節の表を基準に、筆者が選んだ三つだ。三つとも、ラベルを事前に集められず、誰も見ていない場所で回り、間違えたときに巻き戻す道がある。

1. **エージェント harness のコマンド実行ゲート。** ツール呼び出しひとつを `readonly`、`destructive`、`privileged`、`exfiltration` に判定して、人に尋ねるかどうかを決める。コマンドは毎回新しい分布なのでラベルがたまらず、間違えても確認プロンプトに落ちるので誤差予算を決めやすい。末尾の参考資料の themsquared ベンチマークが60件で91.7%を出したが、n が小さくキャリブレーションは言えない。自分のセッションログで測り直すのが最初の段階だ。
2. **browser agent の action 選択。** ページの action space を選択肢として渡し、次のクリックを選ぶ。Browser Use の事例のようにタイピングは小さな LLM に任せ、選択だけを Jev がする。ステップごとに新しい DOM なので分類器を学習させられない場所だ。
3. **確率をそのまま見せる分岐 UI。** 次の質問を選ぶフォーム、フィードを並べ直すスライダーのように、ユーザーが確率を目で見て最終判断を下す画面。しきい値がないので、この記事の罠を避けられる。

逆に、メールや書類や広告の分類のようにラベルが毎日たまる場所は、最初の週は Jev が楽だが、数週間後には自分のラベルで学習した分類器の方が安くて正確になる可能性が高い。そして上の三つのどれも、冒頭のゲートが経験した失敗を避けて通れはしない。作ったあと、線を引く前に自分のデータで測らなければならない。
