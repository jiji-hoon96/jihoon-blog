...
emoji: 🎲
title: 'Jev'
seoTitle: 'TypeSafe Jev와 System One 모델, 확률로 임계값을 그어도 되는가'
date: '2026-09-22'
...
이 블로그에는 음차 외래어를 막는 게이트가 있다. `피커`가 아니라 `picker`로 쓰자는 규칙이고, `content/terminology.yml`의 금지 목록 15개를 `scripts/validate-terminology.mjs`가 검사한다. 그중 한글 음차를 영어로 되돌리는 항목은 아홉 개다.

그런데 목록을 넓히지 못하고 있다. `멀티스레드`처럼 합성어 안에 들어간 말을 기계적으로 치환하면 `멀티thread`가 나오고, `콜 스택`이나 `릴리스`처럼 한국어 기술 문서에 이미 정착한 말을 되돌리면 읽는 쪽이 오히려 멈춘다. **기계가 확실히 할 수 있는 절반은 정규식이 하고, 나머지 절반은 필자가 매번 손으로 판단한다.**

그 나머지 절반은 이런 모양의 일이다. 답이 둘뿐이고, 아는 사람은 1초에 판단하고, 아무도 안 보는 자리에서 돌아야 한다. Jev가 이런 판단을 겨냥한다는 소식을 듣고 이 게이트를 떠올렸다.

그래서 이 글은 소개가 아니라 검증이다. **저 모델이 돌려주는 확률을 믿고 코드에서 선을 그어도 되는가.** 결론부터 적으면 그대로는 안 된다. 왜 안 되는지를 아래에서 순서대로 푼다.

## 텍스트를 만들지 않는 모델

공개일은 2026년 9월 15일이다. 회사는 이것을 LLM이라고 부르지 않고 System One Model이라는 새 범주로 부른다. 창업자 Diogo Almeida는 OpenAI에서 [InstructGPT 논문](https://arxiv.org/abs/2203.02155)의 네 번째 저자로 참여했던 사람이다. ChatGPT의 직계 조상이 된 그 방법을 만든 팀에 있었고, 지금은 그 방법의 한계를 지적하는 쪽에 서 있다.
...
왜 빠른지는 [토큰의 원리](/260610)에서 정리한 prefill과 decode의 비대칭으로 설명된다. LLM이 느린 이유는 출력을 한 토큰씩 순차적으로 짜내기 때문인데, Jev는 그 decode 단계가 없다. 입력을 한 번 병렬로 읽고 확률을 바로 읽어낸다. 그래서 출력 토큰 요금이 아예 없고 입력만 백만 토큰당 $0.042다.
...
RLHF(reinforcement learning from human feedback)의 계보에 답이 있다. 사람의 선호 비교로 보상 모델을 세우는 골격은 [Christiano 외의 2017년 논문](https://arxiv.org/abs/1706.03741)에서 나왔고, [Stiennon 외가 2020년](https://arxiv.org/abs/2009.01325)에 언어 모델에 적용했으며, InstructGPT가 지시 따르기로 확장했다. 세 논문이 공유하는 목표 함수는 하나다. **사람 평가자가 더 선호하는 출력을 내는 것.**
...
OpenAI도 같은 것을 자기 보고서에 적었다. [GPT-4 기술 보고서](https://arxiv.org/abs/2303.08774)의 Figure 8은 사전학습 모델과 post-training 모델의 캘리브레이션 곡선을 나란히 놓는데, 캡션이 이렇다.
...
다만 RLCD로 공개된 것은 출력 계약 세 줄이 전부다. 이 글을 쓰는 2026년 9월 23일 기준으로 TypeSafe의 문서와 발표 글 어디에도 논문이나 기술 보고서가 없다. 같은 약어를 쓰는 [ICLR 2024 논문](https://arxiv.org/abs/2307.12950)이 따로 있는데 그쪽은 reinforcement learning from contrastive distillation이라 다른 방법이다. 검색할 때 섞이지 않게 주의한다.

## confidence의 정체

그래서 Jev가 돌려주는 숫자는 정직한가. 답하기 전에 먼저 볼 것이 있다. **돌려주는 숫자가 하나가 아니다.**

![Jev의 응답에서 probabilities는 학습으로 만들어지고 confidence는 그 분포를 산술로 접은 값이며, 캘리브레이션 주장은 probabilities에만 걸린다는 것을 보여주는 그림](1.png?w=720)
...
이 값은 모델이 만든 것이 아니다. TypeSafe 조직이 공개한 [system-one-adapter-python](https://github.com/typesafe-ai/system-one-adapter-python)의 `_utils/confidence_metrics.py`는 전부 32줄이고, `Choice` 부분이 이렇다.
...
같은 지표를 여러 집합에서 보면 폭이 더 분명하다. 오차 예산 5%에서 자동 처리할 수 있는 비율이 `scienthoon` 집합에서 0.486, `transfer-v9`에서 0.695, `semif`에서 1.000이다. Kev의 채점기는 이 값을 표본 안에서 임계값을 고른 최댓값이라고 적어 두었고, 배포 후의 오차 보장이 아니다. **어느 하나를 모델의 스펙처럼 인용하면 안 된다.**

## 음차 판별 게이트

남의 벤치마크를 읽는 것과 내 데이터에서 재보는 것은 다르다. 그래서 서두의 게이트를 실제로 넘겨 봤다.

필자는 아직 Jev API 키가 없다. 대신 앞서 언급한 **Kev-9B를 로컬에서 돌렸다.** Qwen3.5-9B-Base에 rank-16 LoRA와 pointer head를 얹은 재현 구현이고 Apache-2.0이다. 아래 숫자는 Jev의 숫자가 아니다. Kev의 학습 데이터는 영어 결정 과제이고, 같은 저자의 측정에서 Kev-9B는 영어 과제에서도 Jev에 뒤진다. 오차 예산 5%의 자동화 비율이 0.45에서 0.57인데 Jev가 0.70이고, MMLU-Pro는 0.52 대 0.84다. 그리고 TypeSafe 문서도 Jev에 대해 [영어가 주 학습 언어이고 CJK는 동등하지 않다](https://docs.typesafe.ai/models)고 밝히고 있다.

평가 집합은 이 리포의 한국어 글 18편에서 뽑았다. 라벨의 정답 기준은 **이 리포가 그 단어를 일관되게 어느 표기로 쓰는가**다. 한국어 기술 문서 공동체의 합의가 아니라 이 블로그의 관습이라는 뜻이고, 둘이 갈리는 단어에서는 모델이 공동체 기준으로 맞고 이 기준으로 틀릴 수 있다.

- **게이트 집합 102문장.** 게이트가 이미 아는 단어다. 본문이 영어로 쓴 `calendar`, `picker`, `adapter` 같은 단어를 한글 음차로 되돌린 **반사실 문장**이 양성 23개, `write-post.md`가 예외로 명시한 `리렌더링`이나 `콜 스택`이 실제로 쓰인 문장이 음성 79개다. 여기서 현재 정규식 게이트는 정의상 100%를 맞힌다.
- **보류 집합 60문장.** **게이트가 한 번도 본 적 없는 단어다.** 본문이 영어로만 쓰는 `loader`, `mutation`, `prefill`을 음차로 되돌린 것이 양성 30개, 본문이 한글로만 쓰는 `리듀서`, `스냅샷`, `런타임`이 음성 30개다. 여기서 정규식은 양성을 하나도 못 잡는다.

양성이 필자가 만든 반사실 문장이고 음성이 실제 문장이라는 비대칭은 먼저 밝혀 둔다. 그리고 **유효 표본은 문장 수가 아니라 단어 수다.** 라벨이 단어 단위로 정해지므로 같은 단어가 나오는 문장들은 독립 관측이 아니다. 게이트 집합은 24종, 보류 집합은 13종이다.
...
마지막 줄이 이 실험의 답이다. **보류 집합에서는 임계값을 아무리 높게 잡아도 5% 오차 안에서 자동 처리할 수 있는 결정이 한 건도 없었다.**

예측 분포를 보면 무슨 일이 일어났는지 나온다. 모델은 **162문장 전부에 「되돌려야 한다」고 답했다.** 단어 37종 전부다. 양성은 전부 맞고 음성은 전부 틀렸다. 그래서 보류 집합의 0.500은 실력이 아니라 집합이 균형 잡혀 있어서 나온 숫자다.

그러면서 평균 확신도가 0.876이었다. 게이트 집합에서 확신도 0.9를 넘은 것이 49%인데 그 구간의 실제 적중은 0.240이다.

구조적 항등식도 성립하지 않았다. 「되돌려야 하는가」와 「그대로 둬야 하는가」의 확률을 더하면 게이트 집합에서 평균 1.655가 나왔고, 102문장 전부가 1에서 0.1 넘게 벗어났다.
...
`write-post.md`에 이미 적혀 있는 규칙 문단을 `state`에 함께 넣고 다시 돌렸다. 입력이 문장당 100토큰에서 450토큰으로 늘었다. **게이트 집합은 누출이다.** 그 참고 자료가 게이트 단어와 예외 단어를 이름으로 나열하기 때문이다. 그래서 그쪽은 「참고 자료를 읽기는 하는가」를 보는 대조군이고, 진짜 시험은 보류 집합이다.

| | 게이트(누출) | 게이트 +규칙 | 보류 | 보류 +규칙 |
|---|---|---|---|---|
...
**균형 잡힌 보류 집합에서 문장 단위 정확도가 0.500에서 0.500으로, 전혀 움직이지 않았다.** 단어 단위로는 7/13에서 5/13으로 오히려 내려갔다.

게이트 집합의 0.225에서 0.676은 실력 향상이 아니다. 전에는 102문장 전부에 「되돌린다」고 했고, 참고 자료를 주자 12문장에만 그렇게 답했다. 음성이 0/79에서 68/79로 올랐지만 양성이 23/23에서 1/23으로 무너졌다. 그 집합은 음성이 79대 23으로 많으니 「대체로 두라」는 상수가 자동으로 더 높은 점수를 받는다. **구별을 배운 것이 아니라 기울기를 바꾼 것이다.**
...
한 가지는 눈에 띄게 좋아졌다. 「되돌린다」와 「그대로」의 확률 합이 보류 집합에서 1.508에서 0.963으로 붙었고, 1에서 크게 벗어난 문장이 59개에서 18개로 줄었다. **그런데 정확도는 움직이지 않았다.** 두 질문의 확률이 서로 맞아떨어지는 것과 그 확률이 맞는 것은 다른 일이다.
...
**분포 안에서는 정규식이나 맞춰 놓은 분류기가 이기거나 비긴다.** 필자의 게이트 집합에서 정규식이 1.000 대 0.225로 이긴 것과 같은 자리다. 라벨을 모을 수 있으면 작은 모델을 학습시키는 편이 더 싸고 더 빠르고 더 정확하다. 그건 예전부터 하던 일이다.

**격차는 분포가 새롭고 라벨이 없을 때 벌어진다.** 분포 밖 스팸에서 98.6% 대 73.0%다. 맞춰 놓은 분류기는 자기가 본 적 없는 모양 앞에서 무너지는데 이쪽은 버틴다. 결정 모델의 자리는 **데이터를 모으고 라벨을 붙여 학습시킬 수 없는 판단**이다. 매번 상황이 새로워 라벨을 못 모으는데 판단은 초 단위로 내려야 하는 곳이다.
...
**첫째, 분류와 라우팅.** 이메일 1,500건 분류, 세금 서류 분류, 경쟁사 광고 1,891건을 19초에 $0.12로 분류, PR diff 하나에 typed check 14개, Android 알림에서 광고 판별, 건설 도면 26장을 2.9초에 분류. 서두의 음차 게이트와 같은 모양이다. 앞 절의 표가 말한 대로 라벨이 매일 쌓이는 자리라, 시간이 지나면 자기 라벨로 학습한 작은 분류기가 이기거나 비긴다. Jev의 이점은 라벨이 없는 첫날에 있다.
...
그러면 무엇을 만들어 볼 만한가. 위 세 모양과 앞 절의 표를 기준으로 필자가 고른 셋이다. 셋 다 라벨을 미리 모을 수 없고, 아무도 안 보는 자리에서 돌고, 틀렸을 때 되돌릴 길이 있다.

1. **에이전트 harness의 명령 실행 게이트.** 도구 호출 하나를 `readonly`, `destructive`, `privileged`, `exfiltration`으로 판정해 사람에게 물을지 정한다. 명령은 매번 새 분포라 라벨이 안 쌓이고, 틀리면 확인 프롬프트로 떨어지므로 오차 예산을 정하기 쉽다. 하단 참고 자료의 themsquared 벤치마크가 60건에서 91.7%를 냈지만 n이 작아 캘리브레이션은 말할 수 없다. 자기 세션 로그로 다시 재는 것이 첫 단계다.
2. **browser agent의 action 선택.** 페이지의 action space를 선택지로 주고 다음 클릭을 고른다. Browser Use 사례처럼 타이핑은 작은 LLM에 맡기고 선택만 Jev가 한다. 스텝마다 새 DOM이라 분류기를 학습시킬 수 없는 자리다.
3. **확률을 그대로 드러내는 분기 UI.** 다음 질문을 고르는 폼, 피드를 다시 정렬하는 슬라이더처럼 사용자가 확률을 눈으로 보고 최종 판단을 내리는 화면. 임계값이 없으니 이 글의 함정을 피한다.

반대로 이메일과 서류와 광고 분류처럼 라벨이 매일 쌓이는 자리는 첫 주에는 Jev가 편하지만, 몇 주 뒤에는 자기 라벨로 학습한 분류기가 더 싸고 정확할 가능성이 높다. 그리고 위 셋 중 어느 것도 서두의 게이트가 겪은 실패를 비켜 가지 못한다. 만든 다음 선을 긋기 전에 자기 데이터로 재야 한다.
