...
![Stagehand 的 act() 使用 Jev 的流程。从指令和无障碍树生成候选列表，问 Jev「是哪一个」和「是否都不是」，「都不是」概率低就点击该元素，高就交给 LLM。147 次操作中 143 次由 Jev 完成](1.png?w=720)

那个 PR 划的线，就是这篇文章的问题。它把接受 Jev 答案的标准放在[概率 0.7](https://www.browserbase.com/blog/what-is-jev)，过不了就交给 LLM。**能不能相信那个模型返回的概率，在代码里划一条线。**
...
公开日期是 2026 年 9 月 15 日。公司不把它叫作 LLM，而是归进 System One Model 这个新类别。本文称之为决策模型。创始人 Diogo Almeida 曾在 OpenAI 以第四作者的身份参与过[InstructGPT 论文](https://arxiv.org/abs/2203.02155)。他待过打造了 ChatGPT 直系祖先那套方法的团队，如今站在指出这套方法局限的一边。
...
为什么快，可以用[token 的原理](/260610)一文中梳理过的 :term[prefill]{key="prefill"} 与 decode 的不对称来解释。LLM 慢的原因在于输出要一个 token 一个 token 地顺序挤出来，而 Jev 没有 decode 这个阶段。它并行读一次输入，直接把概率读出来。所以完全没有输出 token 的费用，只有输入按每百万 token $0.042 计费。
...
![图示：在 Jev 的响应里 probabilities 由训练生成，confidence 是把那个分布用算术折起来的值，而校准主张只挂在 probabilities 上](3.png?w=720)
...
![示意图：把 p_max 刻度上的 1/K 拉到 0、1 拉到 1，得到 confidence 刻度。三个选项时 0.80 落在 0.7，1/3 以下被丢弃，顺序不变](4.png?w=720)
...
![图表：Jev 实际调用的 7 个评估集合里，平均确信度集中在 0.851 到 0.906 之间，只有准确率从 0.753 到 0.965 移动，二者的差距与 ECE 几乎相等](5.png?w=720)
...
把同一个指标放到多个集合上看，幅度就更清楚。:term[误差预算]{key="error-budget"} 5% 下可自动处理的比例，在 `scienthoon` 集合是 0.486，`transfer-v9` 是 0.695，`semif` 是 1.000。Kev 的评分器注明这个值是在样本内选取阈值得到的最大值，并不是部署后的误差保证。**不能把其中任何一个当作模型的规格来引用。**

## 音译判别闸门

读别人的基准和在自己的数据上测，是两回事。所以笔者把开头那道闸门实际交给它过了一遍。

笔者还没有 Jev 的 API key。于是改成**在本地跑了前面提到的 Kev-9B。** 它是在 Qwen3.5-9B-Base 上加了 rank-16 :term[LoRA]{key="lora"} 和 pointer head 的复现实现，采用 Apache-2.0。下面的数字不是 Jev 的数字。Kev 的训练数据是英语决策任务，在同一位作者的测量里，Kev-9B 即使在英语任务上也不如 Jev。误差预算 5% 的自动化比例是 0.45 到 0.57，而 Jev 是 0.70；MMLU-Pro 是 0.52 对 0.84。而且 TypeSafe 文档也就 Jev 说明了[英语是主要训练语言，CJK 并不对等](https://docs.typesafe.ai/models)。
...
笔者把 `write-post.md` 里已经写好的规则段落一起放进 `state`，重新跑了一遍。输入从每句 100 token 增加到了 450 token。**闸门集合是:term[泄漏]{key="data-leakage"}。** 因为那份参考资料把闸门词和例外词逐个点名列了出来。所以那一侧是用来看“它到底读不读参考资料”的对照组，真正的考试是保留集合。
...
![在闸门外的 60 句上，把规则段落放进 state 前后的对比。之前 30 句该还原的全对、30 句该保留的全错；之后分别对了 8 句和 22 句。答对总数两次都是 30](7.png?w=720)
...
**差距是在分布是新的、又没有标签的时候拉开的。** :term[分布外]{key="out-of-distribution"}的垃圾邮件上是 98.6% 对 73.0%。调好的分类器在自己没见过的形状面前垮掉，而这一侧撑住了。决策模型的位置，是**无法收集数据、打上标签再训练的判断**。每次情况都是新的所以收不到标签，判断却要以秒为单位做出的地方。
