...
![Como o act() do Stagehand usa o Jev. Uma lista de candidatos é montada a partir da instrução e da árvore de acessibilidade, o Jev é perguntado qual é e se nenhum se aplica, o elemento é clicado quando a probabilidade de «nenhum» é baixa e o passo cai para um LLM quando é alta. 143 de 147 ações terminaram com o Jev](1.png?w=720)

A linha que esse PR traçou é a pergunta deste texto. Ele aceita a resposta do Jev a partir de uma [probabilidade de 0,7](https://www.browserbase.com/blog/what-is-jev) e, abaixo disso, repassa para um LLM. **Dá para confiar na probabilidade que esse modelo devolve e traçar um limiar no código?**

O benchmark dos outros não basta para decidir, então quero verificar com os meus próprios dados. Este blog tem um gate que barra estrangeirismos transliterados. A regra manda escrever `피커` como `picker`, mas a regex só pega a metade certa, e palavras que aparecem dentro de um composto ou já estão consolidadas em coreano, como `멀티스레드` ou `콜 스택`, eu julgo à mão toda vez. Há só duas respostas, reverter ou manter, quem sabe decide em um segundo, e o passa ou falha precisa ser decidido dentro de um script de build sem ninguém conferindo. É o mesmo formato da decisão de selector do Stagehand.

Então a ordem é esta: o que o Jev devolve, de que treinamento sai essa probabilidade, que números aparecem quando entrego meu gate a ele e onde esses números podem ser usados.

## Um modelo que não gera texto

A data de lançamento é 15 de setembro de 2026. A empresa não chama isso de LLM, mas de System One Model, uma categoria nova. Neste texto, chamo isso de modelos de decisão. O fundador, Diogo Almeida, foi o quarto autor do [artigo do InstructGPT](https://arxiv.org/abs/2203.02155) na OpenAI. Ele esteve no time que criou o método que se tornou o ancestral direto do ChatGPT e hoje está do lado que aponta os limites desse método.
...
Por que é rápido se explica pela assimetria entre :term[prefill]{key="prefill"} e decode que organizei em [o princípio dos tokens](/260610). O LLM é lento porque espreme a saída um token por vez, em sequência; o Jev não tem essa etapa de decode. Ele lê a entrada uma vez, em paralelo, e lê a probabilidade direto. Por isso não existe cobrança por token de saída: só a entrada, a $0.042 por milhão de tokens.
...
![Na resposta do Jev, probabilities nasce do treino e confidence é essa distribuição dobrada por aritmética; a afirmação de calibração vale apenas para probabilities](3.png?w=720)
...
![Diagrama da escala de p_max esticada até a escala de confidence, levando 1/K a 0 e 1 a 1. Com três alternativas 0,80 vira 0,7, tudo abaixo de 1/3 é descartado e a ordem não muda](4.png?w=720)
...
![Gráfico de sete conjuntos de avaliação com chamadas reais ao Jev: a confiança média se concentra entre 0,851 e 0,906 enquanto só a acurácia se move de 0,753 a 0,965, e essa diferença quase coincide com o ECE](5.png?w=720)
...
![Antes e depois de colocar o parágrafo de regras no state, nas 60 frases fora do gate. Antes acertou as 30 frases a reverter e errou as 30 a manter; depois acertou 8 e 22. O total de acertos é 30 nas duas vezes](7.png?w=720)
...
**A distância se abre quando a distribuição é nova e não há rótulos.** No spam :term[fora da distribuição]{key="out-of-distribution"}, 98.6% contra 73.0%. O classificador ajustado desmorona diante de um formato que nunca viu, e este lado aguenta. O lugar dos modelos de decisão é **a decisão para a qual não dá para juntar dados, rotular e treinar**. É onde a situação é nova toda vez, os rótulos não se acumulam e mesmo assim a decisão precisa sair em segundos.
