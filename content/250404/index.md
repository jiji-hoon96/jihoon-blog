...
이렇게 하게 되면 변경되지 않은 패키지를 불필요하게 배포하지 않아도 되고, 공통 유틸리티 vs 도메인별 모듈 간 릴리즈 흐름 분리가 쉬우며, 팀 간 병렬적인 개발/배포가 가능하다.

단, 패키지 간 내부 의존성이 있을 경우, **A 패키지의 업데이트가 B 패키지를 참조하는 프로젝트에 자동 반영되지 않도록 주의**해야 한다. 이를 위해 version range 설정(`^1.0.0`)과 CI 테스트가 중요하다.
...
Node.js 13 이상에서는 `exports` 필드를 통해 **모듈 import 경로를 명시적으로 제한**할 수 있다. 이는 단순한 엔트리포인트 설정이 아닌, 패키지 내부 구조를 외부로부터 보호하고, 런타임 환경에 따라 최적화된 모듈을 선택할 수 있게 한다.

--- 

키 | 설명
|----------|------|
**types** | TypeScript에서 import 시 참조할 타입 선언 파일
**browser** | 브라우저 번들 환경에서 우선적으로 참조할 모듈
**default** | 기타 환경(Node.js 등)에서 사용할 기본 모듈

---


또한 exports를 사용하면 하위 경로(./utils/logger)와 같은 직접 접근이 불가능하다.
이는 내부 구현을 캡슐화하여 무분별한 import를 방지하고, 패키지 유지보수성을 높이는 데 매우 유리하다.

참고로, exports가 선언된 경우에는 반드시 여기 명시된 경로만 import 가능하다. 그렇지 않으면 ERR_PACKAGE_PATH_NOT_EXPORTED 오류가 발생하게 된다.
