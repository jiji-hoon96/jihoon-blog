...
이론적으로 두 알고리즘은 "전체 데이터를 압축 해제할 경우" 동등한 능력을 가진다는 것이 증명되어 있다. 그럼에도 LZ77이 살아남은 이유는, **사전을 데이터에 내장한다는 설계가 구현과 확장 양면에서 더 유연했기 때문**이다. 슬라이딩 윈도우의 크기, 매치 검색 알고리즘, 뒤에 붙일 엔트로피 코더 등을 자유롭게 조합할 수 있어서, 시대의 요구에 맞춰 계속 진화할 여지가 있었던 것이다.
