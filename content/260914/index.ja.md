...
このブログもNext.jsの`Link`で記事一覧から記事へ入る。2026-09-14に`web-vitals`を6.2.1に上げて`reportSoftNavs`を有効にした後（`cc21a0d`）、プロダクションのページを開いたヘッドレスChromeにCDPで接続し、GA4へ出ていくリクエストをそのまま開いてみた。一つのセッションで出ていった値はこうだった。（表にない指標はこのセッションのリクエストになかったものだ。特に一覧ページのCLSは、最初のsoft navigationの瞬間に値が0でも一度は報告されるはずだ。`web-vitals`の報告関数は、最初の報告なら0でも送るからだ。キャプチャがGA4のバッチ送信より先に終わった可能性が高いと見ているが、推測であり確認はしていない）
