...
理由はバンドルサイズだった。だからといって、ブラウザの中で起きていることをまったく見ないという意味ではなかった。ブラウザはSDKがなくても、読み込みとレンダリングについてかなり多くのことを自ら記録している。
...
このブログでは読み込み性能がそのまま訪問者の体験であり、約79KBを払うのは筆者ではなく訪問者だ。個人ブログのブラウザエラーではそのコストを回収できないと判断した。ただ、こう決めたからには、ブラウザ側は別の方法で見なければならない。その出発点が、ブラウザがすでに残している記録だ。
...
一つの落とし穴はcross-originのリソースだ。W3Cの[Resource Timing仕様](https://www.w3.org/TR/resource-timing/)では、別のoriginのリソースは、提供するサーバーが`Timing-Allow-Origin`レスポンスヘッダーで許可しない限り、DNS、接続、リクエストとレスポンスの開始といった詳細なタイムスタンプが0で隠される。外部CDNの画像が遅い気がして開いてみたらDNSも接続もすべて0だったなら、速かったのではなく見る権限がなかったのだ。**この領域では、0は速いという意味ではないかもしれない。** 逆に膨らむ値もある。隠されない`responseEnd`から0になった`responseStart`を引くと、本文の転送時間ではなく、ページの開始から応答が終わるまでの時刻が出てくる。サイズのフィールドにも同じ仕様で別の条件がある。`encodedBodySize`と`decodedBodySize`はCORSを通過していないcross-originの応答なら0になり、`transferSize`は`Timing-Allow-Origin`とCORSの両方の影響を受ける。

このブログの最初のバイトまでの時間も短くはない。2026-09-16に韓国のある地点から`curl`で記事二本とホームを一回ずつリクエストしたところ、`time_starttransfer`は0.95秒から2.43秒の間だった（DNS、接続、TLSの時間を含む値だ）。レスポンスヘッダーはNetlify Durableキャッシュがhit、エッジキャッシュがmissだった。サンプルが三つだけなので一般化はしないが、後で見る実測のTTFB 798msと同じ規模だ。こうした区間の分解があって初めて、LCPが遅いときに画像を小さくするのか、ドキュメントの到着を早めるのかを選べる。
...
このブログもNext.jsの`Link`で記事一覧から記事へ入る。2026-09-14に`web-vitals`を6.2.1に上げて`reportSoftNavs`を有効にした後（`cc21a0d`）、プロダクションのページを開いたヘッドレスChromeにCDPで接続し、GA4へ出ていくリクエストをそのまま開いてみた。一つのセッションで出ていった値はこうだった。
...
表に一覧ページのCLSがないのは、確認できなかった箇所だ。その値は最初のsoft navigationの瞬間に0でも一度は報告されるはずだ。`web-vitals`の報告関数は、最初の報告なら0でも送るからだ。キャプチャがGA4のバッチ送信より先に終わった可能性が高いと見ているが、確認はしていない。
...
だから、オプションを有効にする前後で最初の読み込みの指標の分布が変わりうる。同じサイトを測定していて、デプロイの時点を境にINPが良くなったなら、コードが良くなったのではなく観察の窓が短くなっただけかもしれない。Chromium 151以上でのみこう動作するので、ブラウザごとの違いも生じる。

表の値よりも、この一文のほうが解釈に大きく影響する。筆者はこの違いを実測の前に知っておくべきだった。

### bfcacheの復元も新しい体験だ

ページの境界を曖昧にする経路がもう一つある。:term[bfcache]{key="bfcache"}は、戻る・進むの移動のときにページをメモリから丸ごと復元する。web.devの[bfcacheのドキュメント](https://web.dev/articles/bfcache)は、Chromeの利用データでデスクトップの移動10件に1件、モバイルの5件に1件が戻る・進むの移動だと書いている。復元は新しい読み込みではないので、復元を別に数えない収集ツールでは、本来最も速かったはずの再訪問が読み込みの分布から抜け、実際の体験は良くなったのに収集された分布は遅い側に傾くことがある。同じドキュメントは、TTFBのような指標をnavigation typeで分けて見るよう勧めている。`web-vitals` 6.2.1は逆に復元を除外しない。インストールされたコードを開いてみると、復元のときにTTFBを0として新たに報告し、FCP、LCP、CLS、INPも新しい指標として始め直し、このとき`navigationType`は`back-forward-cache`になる。つまりこのブログのTTFBには、soft navigationの0だけでなくbfcache復元の0も混ざる。幸い、soft navigationのために入れたパラメータが両方をまとめて区別してくれる。
...
| `value` | 指標の値を四捨五入した整数。CLSだけ1000を掛ける。GA4は小数のvalueも受け付けるので必須ではなく、Universal Analytics時代のサンプルの整数の慣例と同じ形だ |
...
| `page_location` | 値が測定した画面のURL（`navigationURL`があるときだけ上書きする） |

`page_location`の行は2026-09-16に追加した（`597ca5b`）。前の節で見たとおり、soft navigationが起きると一覧ページのCLSとINPはURLが変わった後に確定して報告される。`web-vitals`のコードも、`soft-navigation` entryを受け取った瞬間に前の画面のCLSを強制的に報告し、新しい指標を始める。gtagはイベントに送信時点のURLを付けるので、上書きしなければ一覧ページの値が記事のURLで記録される。READMEのGA4の例が`page_location: navigationURL`を入れている理由もこれだ。このブログは最初`navigationType`だけを送っていたので、それ以前に溜まったGA4のデータでは、一覧ページのCLSとINPが記事のURLに付いている可能性がある。
...
送らないものもはっきりしている。`web-vitals`のattributionビルドではなく標準ビルドなので、LCP要素が何だったのか、INPの三区間がそれぞれどれだけだったのか、どの要素がレイアウトを押し出したのかは収集しない。前の節のINPの図を思い出すと、このブログは三区間の合計だけを知っていて、どの区間が長かったのかは知らない。そしてブラウザでだけ起きるJSエラーも、どこにも記録されない。約79KBを節約した代償だ。

この値をGA4で実際に分けて読めるかどうかは、最後の記事で扱う。送ることと分けて読めることは別の問題だ。
...
そしてこの計算ルールはページという単位を前提にしている。soft navigationを有効にすると新しい画面の指標が生まれる代わりに、最初のページの観察の窓が短くなり、TTFBには0が混ざる。bfcacheの復元もTTFBの0として混ざる（収集ツールによっては丸ごと抜ける）。筆者が今回新たに理解したのは、オプション一つが値だけでなく**何を一回の体験として数えるか**を変えるという点だ。だから数字を比較する前に、その数字がどんな境界で切られたのかを先に見なければならない。
