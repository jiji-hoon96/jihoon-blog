...
필자의 블로그가 그 선택의 작은 예다. 이 블로그는 `WebVitalsReporter`라는 클라이언트 컴포넌트 하나가 `web-vitals`를 동적으로 불러와 LCP, INP, CLS, FCP, TTFB를 측정하고, GA4에 `web_vitals`라는 단일 이벤트로 보낸다. 지표 이름은 `event_label`로 구분하고, 같은 페이지 수명에서 갱신된 값을 중복 집계하지 않도록 `metric.id`를 함께 싣는다. GA4 이벤트의 value는 정수라서 CLS는 1000을 곱해 반올림한다. 별도 수집 서버 없이 이미 운영하던 GA4에 얹은 구성이라 전용 RUM 제품보다 화면은 부족하지만, 페이지와 디바이스별 분포를 보는 데는 충분했다. 물론 위의 편향도 그대로 물려받는다. 광고 차단기가 GA 요청을 막으면 그 방문자는 필자의 분포에서 빠진다.
