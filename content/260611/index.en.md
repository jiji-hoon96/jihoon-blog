...
![The usage object from an Anthropic SDK response, showing input_tokens, cache_creation_input_tokens, cache_read_input_tokens, and output_tokens](1.png)
...
![The same question called with no MCP servers and with 27 attached: input tokens go from 41 to 10,335 and the per-call cost rises about 11.7 times](6.png)
...
![The first page of the "Lost in the Middle" paper, showing the U-shaped curve where accuracy bottoms out when the answer sits in the middle of the context](4.jpg)
...
![A context rot chart: as input length grows, accuracy falls for Claude, Qwen, OpenAI, and Gemini alike](5.png)
...
![An agent loop diagram: it starts from the user prompt, repeats tool requests and permission checks, and passes through compaction when context pressure builds](8.png)
...
![Model performance plotted against log cost, with the ideal router sitting at the low-cost, high-performance corner](13.png)
...
![Cursor's published chart of CursorBench 3.1 score against average cost per task, with Composer 2.5 approaching top scores at the cheapest end](10.webp)
...
![Prompt engineering next to context engineering: a single-turn query carries only a system prompt and user message, while an agent curates documents, tools, and memory and feeds tool results back](14.webp)
