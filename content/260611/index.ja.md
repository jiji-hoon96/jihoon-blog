...
![Anthropic SDK のレスポンスにある usage オブジェクト。input_tokens、cache_creation_input_tokens、cache_read_input_tokens、output_tokens のフィールドが見える](1.png)
...
![同じ質問を MCP なしと MCP 27 個を付けた状態で呼び出した結果。入力トークンが 41 から 10,335 に、1 回の費用が約 11.7 倍に増えた](6.png)
...
![「Lost in the Middle」論文の 1 ページ目。正解を含む文書が文脈の中央にあるとき精度が最も低くなる U 字カーブが載っている](4.jpg)
...
![context rot のグラフ。入力が長くなるほど Claude、Qwen、OpenAI、Gemini の 4 モデルすべてで精度が下がる](5.png)
...
![エージェントループの図。ユーザープロンプトから始まりツール要求と権限確認を繰り返し、コンテキストが逼迫すると compact を経て次の反復に進む](8.png)
...
![費用に対するモデル性能の分布図。対数の費用軸で低コスト高性能の位置に理想的なルーターが置かれる](13.png)
...
![Cursor が公開した CursorBench 3.1 スコアとタスクあたり平均費用のグラフ。Composer 2.5 が最も安い領域で上位スコアに近づく](10.webp)
...
![プロンプトエンジニアリングとコンテキストエンジニアリングの比較図。単発の質問はシステムプロンプトとユーザーメッセージだけだが、エージェントは文書・ツール・メモリを選んで入れ、ツール結果を戻す](14.webp)
