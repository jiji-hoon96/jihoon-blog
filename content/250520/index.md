...
만약 차이점을 비교해보고싶으면 이미지 클릭하면 된다. 그러면 직접 Fiber Reconciler 와 Stack Reconciler의 동작을 볼 수 있다.
...
Fiber 노드의 type과 key는 React 엘리먼트에서와 동일한 목적을 가진다.

type은 해당 노드가 어떤 종류의 컴포넌트인지를 나타낸다. 함수형 또는 클래스형 컴포넌트의 경우에는 그 함수나 클래스 자체를 가리키며, DOM 요소일 경우에는 'div', 'span'과 같은 문자열로 표현된다.

key는 재조정(reconciliation) 과정에서 해당 Fiber 노드를 재사용할 수 있는지를 판단하는 기준으로 활용된다. 효율적인 업데이트를 위해 중요한 역할을 한다.
