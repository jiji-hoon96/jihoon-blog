...
seoTitle: 'Cómo usar Sentry: consultar datos reales con MCP primero'
date: '2026-09-13'
updatedAt: '2026-09-16'
categories: observabilidad Sentry IA
description: 'Consulté con Sentry MCP datos reales antes de usar Logs, Crons o Uptime: un fallo de GA oculto en un 200, un timeout de 338 s y un veredicto por función.'
...
En mi empresa he trabajado durante años con monitoreo de errores basado en Sentry. Cuando llega un issue, abrir el stack trace, acotar el alcance con releases y tags y buscar las condiciones de reproducción es un trabajo que tengo automatizado. Pero, mirando atrás, las funciones que usaba siempre giraban alrededor de eso. Sabía que existían Logs, Crons, Uptime, los custom spans y el profiling, y aun así nunca los activé.
...
Este artículo es el primero de una serie de cuatro sobre observabilidad. Sigue una incidencia que captó la instrumentación de servidor de este blog, registra lo que salió a la luz al volver a abrir esos datos con MCP y da un veredicto sobre dónde usar cada función. La red y el renderizado dentro del navegador continúan en [Observabilidad del navegador](/260914), la CPU y la memoria en [CPU y memoria del navegador](/260915), y la lectura de los datos recolectados junto con el rendimiento de búsqueda en [De la observación al juicio](/260916).
...
El código actual tiene otra forma. El commit `f348d4c` del 17 de agosto reunió el reporte en un único `captureServerException` y limitó las claves de tag a tres: `locale`, `routeKind` y `operation`. Los tags son la unidad de búsqueda y filtrado, así que meter atributos cuyos valores crecen sin límite hace crecer a la vez la :term[cardinalidad]{key="cardinality"} y el costo. La decisión es dejar como tags solo las preguntas que haré una y otra vez.
...
El commit de corrección `927c85b` hizo que todas las llamadas pasaran 5 segundos. Al reproducirlo con un servidor TCP local que nunca responde, el resultado coincidió con la explicación.
...
La última ocurrencia de JIHOON-BLOG-8 fue el 18 de agosto a las 13:45 UTC, y desde entonces hay 0. Viendo solo el gráfico parece que el problema desapareció, pero yo nunca verifiqué la hipótesis ni arreglé nada. Al cruzarlo con el historial de despliegues, ese mismo día (en UTC) el commit `417d3b4` quitó de la portada las secciones de estadísticas de visitantes y de publicaciones populares como parte de la reforma multilingüe. Las pantallas que llamaban a `stats` y `popular` eran exactamente esas dos. El mensaje del commit `5752e09`, que en septiembre eliminó la ruta de publicaciones populares que quedaba, también dice que "la razón directa por la que se detuvieron los eventos es que desaparecieron los puntos de llamada, no el timeout de 5 segundos".
...
El contador de ocurrencias del issue JIHOON-BLOG-8 marca **144**. Si se agrega el mismo issue en el dataset errors a 90 días, salen solo **14**. Esos 14 restantes van del 17 de agosto a las 10:45 al 18 a las 13:45 UTC, casi exactamente dentro de los 30 días previos a la consulta.
...
Es un evento que se generó cuando seguí el procedimiento de verificación local de la documentación del repositorio (`pnpm build` y luego `pnpm start`). `src/lib/sentry-options.ts`, si no hay `SENTRY_ENVIRONMENT`, determina el entorno a partir del `CONTEXT` de Netlify. Es un mecanismo creado para separar los Deploy Previews de producción, pero en local, donde no existe ninguno de los dos, no pasa ningún valor de entorno, y el evento terminó marcado como `production`. Bloqueó las previews, pero no el entorno local. Si configuro alertas basadas en production en este estado, mis experimentos harán sonar las alertas. Por eso añadí `SENTRY_ENVIRONMENT=local` al comando de verificación de la documentación del repositorio. La razón por la que no cambié el valor por defecto en el código es que todavía no confirmé que `CONTEXT` sea siempre visible en el runtime de funciones de Netlify. Si pongo `local` como valor por defecto sin confirmarlo, esta vez los eventos de producción podrían esconderse bajo `local`.
...
| Crons | ¿La tarea programada se ejecutó a tiempo? | Envío de check-ins | 1 incluido, extra $0.78/mes | **Activar** |
| Uptime | ¿La URL da 2xx desde fuera? | Ninguno | 1 incluido, extra $1/mes | Considerar como apoyo |
...
Lo primero que voy a activar es Crons. Este blog recolecta datos de Search Console todos los lunes con GitHub Actions, y si alguna semana esa tarea deja de ejecutarse en silencio, no se produce ni siquiera un error. Porque no es un fallo, sino **la ausencia de un evento esperado**. Tal como indica la [documentación de Crons de Sentry CLI](https://docs.sentry.io/cli/crons/), si se envuelve el comando existente con la forma `sentry-cli monitors run <monitor_slug> --schedule "<cron>" -- <command>`, el inicio y el fin se envían como check-ins, y la autenticación se hace con el DSN del proyecto. Según la documentación de precios, un cron monitor viene incluido por defecto, así que este uso no cuesta nada.

Uptime ofrece un contraste claro. Es una función que golpea periódicamente una URL desde fuera y comprueba si devuelve 2xx, así que **por principio no puede detectar el fallo dentro de una respuesta 200** que vimos antes. Tiene sentido como apoyo para cuando todo el sitio se cae, pero está en otra capa respecto de la incidencia que este blog vivió realmente.
...
### Seer de pago y Agent Tracing que no aplica

Según la [documentación de precios](https://docs.sentry.io/pricing/), Seer es un complemento de pago de $40 al mes por colaborador activo. Esta vez evalué ejecutarlo sobre el issue `InvariantError` interno de Next.js abierto el 11 de septiembre (JIHOON-BLOG-A), pero no lo hice porque es una llamada que toca la facturación. Por eso este artículo no contiene experiencia de primera mano con Seer. Agent Tracing pasó a [GA el 11 de septiembre de 2026](https://sentry.io/changelog/agent-tracing-is-now-ga/). Es un punto en el que es fácil equivocarse y escribir que está en beta si uno se apoya en la memoria de un modelo o en artículos antiguos, pero este blog no tiene rutas de llamadas a LLM, así que no aplica.

## Lo que la IA redujo y lo que no

Los costos que la IA redujo en este trabajo son claros. Reunir condiciones desde documentación dispersa (si el profiling del navegador está en beta, las cabeceras, las restricciones de navegador), aprender la sintaxis de consultas para ir cambiando el group by, el cálculo de restar y sumar marcas de tiempo de breadcrumbs y el borrador de la tabla de funciones: todo se resolvió en unos pocos intercambios. Al bajar la barrera de la exploración, fue posible preguntar primero "qué dicen los datos actuales" antes de decidir "si lo activo o no".

Lo que no redujo es igual de claro.

- **Datos que no se guardaron.** De los 144 eventos, 130 desaparecieron, y los spans que quedaron fuera de la muestra nunca existieron. Un agente no puede restaurar datos que no existen.
