...
Evans 在 2003 年出版的 Domain-Driven Design 中提出了 ubiquitous language，也就是**共享语言**这一模式。它常被简化为“让 developer 和领域专家使用相同的术语”，但读原书会发现处方要具体得多。他要求团队把模型作为语言的骨架，并约定在团队内的一切沟通和代码中坚持使用这套语言。而紧随其后的那句话，对我而言最为重要。
...
那么歧义该怎么发现呢？Evans 对此也给了答案：在对话中反复使用，术语理解上的差异就会显现。我是这样读这句话的。歧义不是靠精读文档找出来的。无论把需求文档读得多仔细，“meter”这个词具有多种含义的事实都不会浮现。只有把这个词放到实际案例上反复使用，它才会分化。像“其他人把 meter 理解成什么意思？”这样的问题冒出来的瞬间，就是那个分岔点。

## 最小协作努力原理

我们可能知道这个方法，却仍然无法实践。那么，为什么不反复去问呢？
...
这一点在实务中会成为调节确认力度的依据。在还在探索方向的场合，抓着“这里的 meter 是什么意思？”不放，对话就推进不下去。那个阶段需要的是关于要做什么、为什么做的共识，而不是边界值的确定。反过来，在进入实现之前，这个问题就必须提出来。此时双方必须能够相信彼此对“meter”的范围想的是同一件事，否则代码就会堆在错误的前提之上。
...
来看看 Cucumber 项目负责人 Matt Wynne 在 2015 年介绍的[Example Mapping](https://cucumber.io/blog/bdd/example-mapping-introduction/)。他把许多团队觉得需求讨论困难的原因，诊断为缺乏结构导致耗时又无聊。于是团队既不会定期做，也不会一致地做。我正是如此。需求会议一旦拖长，下次就想快点结束，而草草结束，含糊之处就原样留了下来。
...
我认为这个思路是面对沟通问题最现实的路径。与其试图改变人的态度，不如**为那些难以说出口的事情做一个格子。**好的结构不是要求人做得好，而是让人容易做好。
...
Google 的 Project Aristotle 也是常被引用的资料。根据[reWork 上公开的内容](https://rework.withgoogle.com/intl/en/guides/understand-team-effectiveness)，他们调查了 180 个团队，对数百个变量跑了 35 个以上的统计模型，提出影响团队效能的五个因素：心理安全感、可靠性、结构与清晰度、意义，以及影响力。而结论是，比起团队里有谁，这个团队如何协作更为重要。
