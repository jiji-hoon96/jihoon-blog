...
![Biome 공식 홈페이지의 포매팅 속도 비교. 2,104개 파일 171,127줄 기준 Biome 0.41초, Prettier 14.35초로 약 35배 차이](1.png)
...
![Biome 를 쓴 Vite 개발 서버 실행 결과. 506ms 만에 준비됨](biome1.png)  ![ESLint 와 Prettier 를 쓴 Vite 개발 서버 실행 결과. 630ms 만에 준비됨](lint1.png)
...
![Biome 를 쓴 Vite 빌드 결과. 117.13초](biome2.png) ![ESLint 와 Prettier 를 쓴 Vite 빌드 결과. 131.48초](lint2.png)
...
![Biome 의 린트 실행 결과. 0.79초, CPU 0.470초](biome3.png) ![ESLint 의 린트 실행 결과. 16.32초, CPU 8.600초](lint3.png)
...
| ![동시 접속 10, 100, 300 에서의 평균 CPU 사용률 비교. Node.js 는 89~91% 로 거의 고정이고 Rust 는 145~206% 로 여러 코어를 쓴다](5.webp) | ![같은 조건의 평균 메모리 사용량 비교. Node.js 는 64~89MB, Rust 는 6~14MB](6.webp) |
| --- | --- |
...
![동기 처리와 비동기 처리 비교 다이어그램. 동기는 요청과 응답이 번갈아 오지만 비동기는 요청을 연달아 보내고 응답을 나중에 받는다](7.png)
...
![구문 트리 예시. 왼쪽은 단항 연산자 -1, 오른쪽은 이항 연산자 1 + 2 를 연산자 노드와 정수 노드로 나타낸다](4.svg)
...
![npm trends 의 1년 주간 다운로드 그래프. eslint 와 prettier 는 3천만~5천만 대이고 biome 은 축 바닥에 붙어 있다](8.png)
