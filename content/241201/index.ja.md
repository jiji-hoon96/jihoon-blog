...
description: "BiomeのLintとフォーマット性能を、ESLintとPrettierを組み合わせた構成と比較する。Rust製オールインワンツールチェーンを実務に導入した経験をもとに、設定の移行手順、詰まりやすい箇所、置き換えられる範囲と残る課題までまとめた。"
...
![Biome を使った Vite 開発サーバーの起動結果。506ms で準備完了](biome1.png)  ![ESLint と Prettier を使った Vite 開発サーバーの起動結果。630ms で準備完了](lint1.png)
...
![Biome を使った Vite のビルド結果。117.13 秒](biome2.png) ![ESLint と Prettier を使った Vite のビルド結果。131.48 秒](lint2.png)
...
![Biome の lint 実行結果。0.79 秒、CPU 0.470 秒](biome3.png) ![ESLint の lint 実行結果。16.32 秒、CPU 8.600 秒](lint3.png)
...
![あごに手を当てて考え込むパトリックのキャラクター](3.jpeg)
...
| ![同時接続 10、100、300 での平均 CPU 使用率の比較。Node.js は 89〜91% でほぼ一定、Rust は 145〜206% と複数コアを使う](5.webp) | ![同じ条件での平均メモリ使用量の比較。Node.js は 64〜89MB、Rust は 6〜14MB](6.webp) |
| --- | --- |
...
![同期処理と非同期処理の比較図。同期はリクエストとレスポンスが交互に進み、非同期は続けて送って後から受け取る](7.png)
...
![構文木の例。左は単項演算子の -1、右は二項演算子の 1 + 2 を演算子ノードと整数ノードで表す](4.svg)
...
![npm trends の 1 年間の週間ダウンロード数。eslint と prettier は 3,000 万〜5,000 万で、biome は軸の底に張り付いている](8.png)
