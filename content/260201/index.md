...
for (let i = 0; i < arr.length; i++) {
  newArr[i] = `I love ${arr[i]}`;
}
```

이 코드를 읽는 개발자는 `for`문의 초기화, 조건, 증감을 파악하고, 인덱스로 배열에 접근하는 방식을 이해하고, 새로운 배열에 값을 할당하는 과정까지 전부 머릿속에 담아야 한다. 실제로 이 코드가 하는 일은 "배열의 각 요소 앞에 문자열을 붙인다"는 것뿐인데, 그 의도를 파악하기 위해 너무 많은 맥락에 노출되는 것이다.

```javascript
const newArr = arr.map((v) => `I love ${v}`);
```

`Array.prototype.map`이라는 추상화 덕분에 개발자는 "배열을 순회하며 새로운 배열을 만든다"는 동작만 알면 된다. 인덱스 관리, 새 배열 선언 같은 맥락은 사라졌다. 코드가 **어떻게(How) 동작하는지**가 아니라 **무엇을(What) 하는지**에 집중할 수 있게 된 것이다.
...
아이폰을 만든다고 생각해보자. 공통점 추출 방식으로 접근하면 이렇게 된다.

```tsx
// 현재 아이폰의 공통 특성에서 출발한 설계
const IPhone = ({ showHomeButton }: { showHomeButton: boolean }) => {
...
나중에 다이얼 방식의 홈 버튼이 추가되면? `showHomeButton`은 의미가 없어지고, 내부 로직 전체를 뜯어고쳐야 한다. 하지만 부품 조립 방식으로 접근하면 이야기가 달라진다.

```tsx
// 부품을 조립하는 설계
const IPhone = ({ renderHomeUI }: {
...
// 어떤 홈 UI든 외부에서 조립할 수 있다
<IPhone renderHomeUI={(moveHome) => <HomeButton onClick={moveHome} />} />
<IPhone renderHomeUI={(moveHome) => <HomeGesture onSwipeUp={moveHome} />} />
<IPhone renderHomeUI={(moveHome) => <HomeDial onChange={moveHome} />} />
```

"홈으로 이동하는 기능"은 변하지 않는 추상적 개념이고, "홈 버튼의 형태"는 변하기 쉬운 구체적 구현이다. 이 둘을 분리해서 생각하면, `IPhone` 컴포넌트의 내부를 건드리지 않고도 새로운 형태의 홈 UI를 얼마든지 추가할 수 있다.
