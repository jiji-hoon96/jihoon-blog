...
emoji: 🧩
title: "추상화"
seoTitle: "프론트엔드 추상화 — 좋은 코드를 위한 설계 원칙"
date: "2026-02-01"
categories: 프론트엔드 설계 추상화
description: "프론트엔드에서 추상화란 무엇이고, 좋은 추상화와 나쁜 추상화는 어떻게 다른가. 추상화 수준, 의도를 드러내는 이름, 부품 조립 방식, 빈약한 도메인 모델까지 React 컴포넌트와 함수 설계 관점에서 정리한다."
keywords: "프론트엔드 추상화, React 컴포넌트 추상화, Clean Code 추상화, 추상화 수준, Level of Abstraction, 좋은 코드 작성법, 의도를 드러내는 이름, Composed Method, 새는 추상화의 법칙, 컴포넌트 설계, 커스텀 훅 설계, 프론트엔드 아키텍처, Kent Beck, Robert C. Martin"
---
...
**추상(Abstract)** 은 상태이자 속성이다. "이것은 추상적이다"라고 말할 때, 그것은 구체적인 세부사항이 생략되어 있고 **핵심적인 개념만 남아있는 상태**를 뜻한다. Java나 TypeScript에서 `abstract` 키워드가 붙은 클래스나 메서드가 바로 이런 의미이다. 아직 구체적인 구현이 채워지지 않은, 본질적인 형태만 정의된 미완성의 설계도인 것이다.
...
일상에서 "추상적"이라는 말은 흔히 "모호하다"는 뉘앙스로 쓰인다. 하지만 프로그래밍에서의 추상화는 정반대이다. 추상화의 목적은 모호해지는 것이 아니라, 절대적으로 정확할 수 있는 새로운 의미 수준을 만드는 것이다. 주어진 맥락에서 관련있는 정보를 보존하고, 관련 없는 정보를 잊는 행위가 추상화의 본질이 된다.
...
추상화가 프로그래밍에서 필요한 근본적인 이유는 의외로 단순하다. **더 복잡한 것을 만들기 위해서**이다. 더 복잡한 것을 만드려고하는데, 복잡한 요소들이 너무 많으면 그걸 다 기억하고 다루기 어렵기 때문이다. 그래서 우리는 복잡한 요소들을 그룹화해서 단순화한 추상적인 개념으로 만든다.

프론트엔드 개발자가 매일 사용하는 React만 해도 그렇다. 컴포넌트 하나를 렌더링하기 위해 내부적으로는 Virtual DOM 생성, 재조정(Reconciliation), 실제 DOM 조작이라는 복잡한 과정이 일어난다. 하지만 우리는 그런 걸 신경 쓰지 않고 JSX를 작성하면 된다. React가 이 복잡한 과정을 추상화해주었기 때문이다.

아래 코드를 살펴보자. 우리는 UserProfile 컴포넌트를 사용할 때, 내부적으로 발생하는 VDOM 생성, diffing 같은 복잡한 과정에 대해서 몰라도 충분히 UI를 만들고 다룰 수 있다.

```tsx
<UserProfile name="jihoon" />
```

예전에는 Webpack을 직접 설정하는 것이 프론트엔드 개발자의 일상이었지만, 지금은 Next.js나 Vite 같은 프레임워크가 번들링 설정을 추상화해버렸다. 덕분에 우리는 번들러의 내부 동작을 몰라도 애플리케이션을 개발할 수 있게 되었고, 그 시간에 **비즈니스 로직이나 사용자 경험 같은 더 고차원적인 문제에 집중**할 수 있게 되었다. (그렇기에 필자는 프론트엔드 개발자의 역할에서 추상화의 개념이 매우 중요하다고 생각한다.)
...
그렇다면 추상화가 이렇게 좋은 것이라면, 무조건 많이 하면 좋은 걸까? 어떤 목적을 가지고 추상화를 해야 하는지 생각해보자.
...
이 함수를 읽는 사람은 "사용자 등록 흐름"이라는 비즈니스 규칙 기반의 높은 수준의 맥락을 따라가다가, 갑자기 해시 버퍼 조작과 SQL 쿼리와 이메일 템플릿 문자열이라는 낮은 수준의 맥락으로 끌려 내려간다. 그러다 다시 `sendWelcomeEmail`라는 높은 수준으로 점프한다. 이렇게 추상화 수준이 오르락내리락하면, 읽는 사람의 머릿속도 함께 오르락내리락하게 된다.
...
모든 문장이 같은 높이의 추상화 수준에서 이야기하고 있다. 이메일 전송이 어떻게 구현되어있는지, 비밀번호 해싱이 어떤 알고리즘을 쓰는지는 각각의 하위 함수가 책임진다. 이 함수를 읽는 사람은 오직 "사용자 등록의 전체 흐름"이라는 하나의 맥락에만 집중하면 된다.
...
예를 들어보자. 요구사항에 A, B, C라는 버튼이 있고, 세 버튼 모두 파란색 둥근 모양이며 차이점은 라벨 텍스트뿐이라고 하자. 공통점만 뽑아 설계하면 아래처럼 표현할 수 있다. 
...
이 순간 `BlueRoundButton`이라는 이름부터 어색해진다. 색상 prop을 추가하면 되긴 하지만, 애초에 "파란색 둥근 버튼"이라는 구체적인 공통점에서 출발한 설계가 변경에 취약했던 것이다. (이 정도는 양반이다. 현실에서는 버튼의 모양, 버튼의 크기 등 수많은 요구상이 날아온다)
...
물론 경험이 많은 개발자라면 "처음부터 IoC(제어의 역전)로 설계하면 되는 거 아니냐"고 할 수 있다. 맞는 말이다. 하지만 그런 판단이 가능한 이유는 과거에 수많은 삽질을 통해 "어디가 변경되기 쉬운 부분인지"에 대한 감각이 쌓였기 때문이다.
...
`calculateDiscountedPrice`는 이름만 봐도 원가와 할인율을 받아 할인된 가격을 계산한다는 것을 알 수 있고, `number`를 받아서 `number`를 반환한다는 타입 정보가 이를 뒷받침한다. 내부에서 어떤 계산 로직이 적용되는지는 몰라도 된다.

반면 `calculate(price: number, rate: number): number`는 계산하는 대상에대한 정보를 모르니 결과에 대한 예상을 할 수 없다. 결국 소스코드를 까봐야만 사용할 수 있게 되고, 추상화의 장점을 잃어버리는 것이다.
...
> 단, 동사 단독으로 추상화 수준이 결정되는 것은 아니다. 함께 오는 명사(도메인 맥락)가 최종 수준을 결정한다

추상화 수준이 **낮은** 쪽에서 자주 쓰이는 동사들이 있다. `parse`, `encode`, `decode`, `serialize`, `read`, `write`, `push`, `pop`, `convert`, `transform` 같은 것들이다. 이 단어들은 데이터의 물리적인 변환이나 자료구조의 직접적인 조작을 암시한다.

중간 수준에서는 `get`, `save`, `load`, `validate` 같은 동사들이 등장한다. 기술적인 동작이지만, 그 동작의 의도가 어느 정도 드러나는 수준이다.

추상화 수준이 **높은** 쪽에서는 `register`, `refund`, `confirm`, `cancel`, `submit` 같은 동사들이 쓰인다. 이 단어들은 비즈니스 도메인의 언어이다. 내부에서 어떤 기술적 절차가 일어나는지는 전혀 드러나지 않고, **사용자의 행위나 비즈니스 프로세스**만이 표현된다.
...
컴포넌트는 추상화 수준에 따라 이름의 구체성이 달라진다. Button은 낮은 수준에서 범용 UI 프리미티브로 사용되지만, SubmitOrderButton은 높은 수준에서 비즈니스 의도가 명확히 드러난다.
...
`on*`은 컴포넌트가 외부에 노출하는 prop 이름이다. 컴포넌트를 사용하는 쪽에서 "어떤 이벤트에 반응할지"를 선언한다.`handle*`은 그 prop에 실제로 전달되는 구현 함수 이름이다.
...
커스텀 훅은 `use` 접두사를 사용하여 React의 규칙을 따르고, 훅이 제공하는 상태나 동작을 컴포넌트에서 사용할 수 있도록 한다.

> [Coding Horror](https://blog.codinghorror.com/) 을 운영하는 Jeff Atwood 는 `Manager`라는 접미사의 문제를 지적한 적이 있다. `UrlManager`라는 이름은 URL을 풀링하는 건지, 검증하는 건지, 생성하는 건지 전혀 알 수 없다. `UrlBuilder`, `UrlValidator`, `UrlPool`처럼 구체적인 역할을 드러내는 이름이 훨씬 낫다. 이름이 모호하다는 것은, 그 모듈의 책임 자체가 모호하다는 신호일 수 있는 것이다.
...
추상화의 폭은 사용자가 누구인지에 따라 결정되어야 한다. 내부 구현을 이해하고 세밀하게 제어해야 하는 사용자에게는 낮은 수준의 인터페이스가 적합하다. 반면 세부사항을 몰라도 되는 사용자에게 지나치게 열린 인터페이스를 제공하면 혼란만 가중된다. 반대로 다양한 상황에 대응해야 하는 사용자에게 입력을 과도하게 제한하면 사용처 자체가 막혀버린다
...
여기까지 읽으면 "그래서 추상화를 언제 해야 하는 건데?"라는 질문이 남을 수 있다. 필자의 생각은 이렇다. **기본 전제는 추상화를 섣부르게 하지 않는 것이어야 한다.**
...
이런 상황에 빠졌다면, 해법은 명확하다. 추상화된 코드를 각 사용처에 다시 인라인하고, 각 사용처에서 불필요한 코드를 제거한 뒤, 깨끗해진 상태에서 진짜 공통점이 보이면 그때 다시 추상화한다. **"가장 빠른 전진은 뒤로 돌아가는 것이다."**
...
Joel Spolsky가 2002년에 제시한 **"새는 추상화의 법칙(The Law of Leaky Abstractions)"** 도 이 맥락에서 기억해둘 만하다. 새는 추상화의 법칙은 추상화는 복잡한 구현을 숨기려 하지만, 그 구현의 세부사항이 결국 밖으로 새어나온다(leak) 는 것이다. 즉 추상화를 사용하는 사람이 내부 구현을 몰라도 된다고 설계했지만, 실제로는 알아야만 제대로 쓸 수 있는 상황이 생긴다는 뜻으로 사용된다.
...
결국 **"추상화는 우리의 작업 시간을 절약해주지만, 학습 시간을 절약해주지는 않는다."**
...
프로그래밍에서 추상화란 복잡한 것을 감추어 단순해 보이게 만들고, 코드를 읽는 사람이 필요한 맥락에만 집중할 수 있게 해주는 행위이다.

좋은 추상화를 위해 기억해둘 것들을 다시 한번 짚어보자면 이렇다.

- 기본 전제는 섣부르게 추상화를 하지 않고 명확한 신호가 보일 때 비로소 분리하자.
...
이 글은 여러 공식문서와 선행 아티클에서 많은 영감을 받았다. 직접 인용한 구절의 출처와, 사고의 틀을 잡는 데 도움이 된 글들을 함께 남긴다.
...
- 박서진님, 「좋은 코드를 위한 나의 선택, 신중하게 코드 작성하기」 (Toss SLASH 23 발표) : "공통점 추출"이 아닌 **"부품의 조립"** 으로 접근하라는 관점은 이 발표에서 참고했다.
